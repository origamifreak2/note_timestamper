...
const fpsSelect = document.getElementById('fpsSelect');     // Framerate selector dropdown
 ...
 *
 * Usage:
 * 1. Click the drawing button (✏️) in the toolbar
 ...
 *
 * Usage:
 * 1. Click the camera button (📷) in the toolbar next to the image button
...
const LS_KEYS = { mic: 'nt_selected_mic', cam: 'nt_selected_cam', res: 'nt_selected_res', fps: 'nt_selected_fps' };
...
function getSelectedFramerate() {
  const fpsValue = fpsSelect?.value || '30';
...
async function loadDevices() {
  ...
  if (savedFps && [...fpsSelect.options].some(o => o.value === savedFps)) {
...
async function startRec() {
  try {
    statusEl.textContent = 'Requesting devices…';
    ...
    mediaRecorder.onresume = () => { statusEl.textContent = 'Recording…'; };
    mediaRecorder.onerror = (e) => {
      console.error('MediaRecorder error:', e?.error || e);
      statusEl.textContent = 'Recorder error — see console';
    };
...
/**
 * Updates the video-related selector states based on recording and audio-only mode
 * Disables resolution and framerate changes during active recording
 */
function updateResolutionState() {
  ...
  if (fpsSelect) {
    fpsSelect.disabled = isCurrentlyRecording || audioOnly.checked;
    fpsSelect.title = isCurrentlyRecording ?
      'Stop recording first to change framerate' :
      (audioOnly.checked ? 'Framerate not applicable for audio-only recording' : 'Select recording framerate');
  }
}
...
btnSave?.addEventListener('click', async () => {
  ...
  statusEl.textContent = res.ok ? `Saved → ${res.dir}` : 'Save canceled';
});
...
<script>
  ...
  statusEl.textContent = res.ok ? `Exported → ${res.path}` : 'Export canceled';
}
...
<script>
  ...
  if (res.ok) {
    if (res.videoPath) {
      statusEl.textContent = `Exported → HTML + ${res.videoFileName}`;
    } else {
      statusEl.textContent = `Exported → HTML (no video)`;
    }
  } else {
...
// Framerate selection change - save selection (cannot switch live during recording)
